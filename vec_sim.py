...
class VecSim:
    ...
    def simulate(self, 
        ...
        if self.rn.coupling:
            ...
            l_k = self.rn.compute_log_constants(self.rn.kon, self.rn.rxn_score_vec, self._constant)
            if verbose:
                print(f"k_on: {self.rn.kon}")
                print(f"rn.rxn_score_vec: {self.rn.rxn_score_vec}")
                print(f"constant: {self._constant}")
                print(f"l_k: {l_k}")
...
#                         mod_flag=False
            ...
            if cur_time + step*conc_scale > self.runtime:
                ...
                if verbose:
                    print("Species Concentrations: ",self.rn.copies_vec)
